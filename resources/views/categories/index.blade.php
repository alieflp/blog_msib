                ...
                <div class="space-y-4">
                    @foreach ($categories as $category)
                        <div class="flex justify-between items-center bg-white dark:bg-gray-800 shadow rounded-md p-4">
                            <a href="{{ route('categories.show', $category->id) }}" class="text-lg text-gray-900 dark:text-gray-200">{{ $category->name }}</a>
                            <div class="space-x-2">
                                <a href="{{ route('categories.edit', $category->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</a>
